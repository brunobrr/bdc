new_get_names <- function (id, db = getOption("taxadb_default_provider", "itis"), 
                                                 ...
                                                 "bare", "uri"), taxadb_db = taxadb::td_connect()) 
{
  ...
  df <- filter_id(prefix_ids, provider = db, version = version, 
                  collect = FALSE, db = taxadb_db) %>% dplyr::select("scientificName", 
                                                                     "taxonID", "sort", "taxonomicStatus") %>% dplyr::distinct() %>% new_take_first_duplicate() %>% 
    dplyr::collect()
  if (dim(df)[1] != n) {
...
new_take_first_duplicate <- function (df) 
{
