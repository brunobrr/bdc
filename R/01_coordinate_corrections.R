...
for(i in 1:length(URL)){
  ...
  temp <- temp[[1]] %>% dplyr::as_tibble() %>% dplyr::filter(!X1%in%LETTERS, !X1=='English name')
  base::colnames(temp) <- c('english_name', 'names_in_different_languages')
  wiki_cntr[[i]] <- temp
  # wiki_cntr[[i]] <- janitor::clean_names(temp)
  temp <- lapply(wiki_cntr[[i]][,2] %>% pull(1), extract_cntr_names)
  names(temp) <- wiki_cntr[[i]] %>% dplyr::pull(english_name)
...
filt <-
  stringr::str_which(cntr_db$cntr_original,
                     stringr::str_flatten(wiki_cntr_name, collapse = "|"))
cntr_db$cntr_suggested[filt] <- 'BRAZIL' #Assign here your country name in English and in upper case (equal to standardizeCountry() function output)

filt <- stringr::str_which(cntr_db$cntr_original, "Bra|Bré")# Warning: this is only for Brazil 
...
# Filter database only for those occurrences with cntr_iso2c equal to 'BR' this new occ database will be used to try rescue come occurrences georeferenced outside interest country
...
occ2 <-
  occ %>% filter(!is.na(decimal_longitude) |
...
my_country2 <- raster::buffer(my_country, width=0.5) #0.5 degree ~50km near to equator
plot(my_country)
plot(my_country2, add=T)

coord_sp <- sp::SpatialPoints(coord_test %>% dplyr::select(decimal_longitude, decimal_latitude))
coord_sp@proj4string <- my_country2@proj4string
over_occ <- sp::over(coord_sp, my_country2)
coord_test %>% dplyr::filter(over_occ == 1) %>% 
  dplyr::select(decimal_longitude, decimal_latitude) %>% 
...
ggplot()+
  ...
  geom_point(data=coord_test, aes(x=decimal_longitude, y=decimal_latitude)) + geom_point()+
  geom_point(data=coord_test, aes(x=decimal_longitude_modified, y=decimal_latitude_modified), col='red')+
  coord_equal()


# PROCEDURE TO ADD THESE CORRECTED RECORDS TO ORIGINAL DATABASE 
# Arrange  two databases based on ID
coord_test <- coord_test %>% dplyr::arrange(database_id)
# occ <- occ %>% dplyr::arrange(database_id)
occ <- vroom::vroom(temp) %>% dplyr::arrange(database_id) #Occurrence database
...
occ[filt, c("decimal_latitude", "decimal_longitude")] <-
  coord_test[c("decimal_latitude_modified", "decimal_longitude_modified")]
