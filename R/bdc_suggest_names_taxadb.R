...
#' @param max.distance A numeric value specifying the minimum distance between the sci_name and that found in taxadb. The default is 0.75.
...
bdc_suggest_names_taxadb <-
  function (sci_name,
    ...
    sug_dat <- data.frame(original = character(length(sci_name)), suggested = character(length(sci_name)), distance = numeric(length(sci_name)))
