...
#' @param max.distance numeric,  a value between 0 and 1 specifying the minimum distance between the scientific names and names suggested by a fuzzy matching. Values close to 1 indicate that only a few differences between scientific names and name suggested are allowed. Default is 0.9.
  ...
  function (sci_name,
    ...
    if (!is.null(rank_name) & !is.null(rank)) {
      ...
      species.first.letter <-
        ...
        dplyr::filter(., kingdom == rank_name) %>%
        dplyr::pull(scientificName) %>%
        grep(paste0("^", first.letter, collapse = "|"), ., value = TRUE)
    ...
    if (parallel == TRUE) {
      ...
      sug_dat <-
        foreach(i = sci_name, .combine = rbind, .verbose = T, .export = "bdc_return_names") %dopar% {
          bdc_return_names(i, max.distance, species.first.letter)
        } # end foreach
      ...
      for (i in seq_along(sci_name)) {
        sug_dat[i, ]  <-
          bdc_return_names(sci_name[i], max.distance, species.first.letter)
      }
    }
    return(sug_dat)
  }