if (FALSE) {
  ...
  data.table::fwrite(at_epiphytes, "Input_files/at_epiphytes.csv")
  ...
  data.table::fwrite(idigbio, "Data_raw/idigbio/idigbio.csv")
  ...
  # sibbr -------------------------------------------------------------------
  txt_files <-list.files (path = "Data_raw/sibbr",
                          pattern = "*.zip",
                          full.names = T)

  # List files
  zipF <- list.files(path = "Data_raw/sibbr", pattern = "*.zip", full.names = TRUE)

  # unzip all your files
  ldply(.data = zipF, .fun = unzip, exdir = "Data_raw/sibbr")

  # get the csv files
  csv_files <-
    list.files(path = "Data_raw/sibbr",
               pattern = "*.csv",
               full.names = T)

  # read the csv files
  sibbr <- ldply(.data = csv_files, .fun = vroom)
  dim(sibbr)
  data.table::fwrite(sibbr, "Data_raw/sibbr.csv")
