...
Read an example of the **configuration table**. You can download the table by clicking on "csv" button.
...
Note that the standardized database integrating all dataset are saved in the "Output/Intermediate" folder as *"00_merged_database.qs"*. The database is saved with a "qs" extension, a format useful for quickly saving and reading large databases. This files can be read using the function "qread" from the "qs" package (see below).
...
```{r echo=F, message=FALSE, warning=FALSE}
DT::datatable(
  head(database), class = 'stripe', extensions = 'FixedColumns',
  options = list(
    pageLength = 3,
    dom = 'Bfrtip',
    scrollX = TRUE,
