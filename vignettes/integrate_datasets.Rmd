...
knitr::opts_chunk$set(echo = TRUE, warning = FALSE, eval = T)
devtools::load_all()
```
...
The first step of the BDC workflow handles the harmonization of heterogeneous datasets in a standard format in a simple and efficient way. How is this accomplished? Basically, by replacing in the headers of original datasets by standardized terms. To do so, users must fill-out a **configuration table** to indicate which field names (i.e., column headers) of each original dataset match a list of **Darwin Core standard terms** (see table below). 
...
metadata <-
  DatabaseInfo <-
  read_csv(here::here("inst", "extdata", "Config", "DatabaseInfo.csv"))
```

```{r echo = FALSE}
DT::datatable(head(metadata), editable = 'cell')
```
