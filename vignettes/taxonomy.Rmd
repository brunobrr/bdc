...
```{r echo=F}
message("
>> Family names prepended to scientific names were flagged and removed from 20 records.
>> Terms denoting taxonomic uncertainty were flagged and removed from 35 records.
>> Other issues, capitalizing the first letter of the generic name, replacing empty names by NA, and removing extra spaces, were flagged and corrected or removed from 29 records.
>> Infraspecific terms were flagged and removed from 46 records.

The latest gnparser version is v0.14.4
tentando a URL 'https://gitlab.com/gogna/gnparser/uploads/4ce35a78d821297308363cc3f4770f29/gnparser-v0.14.4-win-64.zip'
Content type 'application/zip' length 7947842 bytes (7.6 MB)
downloaded 7.6 MB

gnparser has been installed to C:/Users/Bruno R. Ribeiro/AppData/Roaming/gnparser
...
## Names harmonization
The taxonomic harmonization is based upon **one** of those taxonomic authorities previously mentioned. It starts with the creation of a local database by download, extraction, and importation of the taxonomic database informed by users using taxadb package. The download may take a time depending on the internet connection. 
...
query_names <- bdc_query_names_taxadb(
  ...
  parallel = TRUE,
  ncores = 2,
  export_accepted = FALSE # multiple accepted names 
)
```
