...
This step of the `bdc` package extracts the collection year whenever possible from complete and legitimate date information and flags dubious (e.g., 07/07/10), illegitimate (e.g., 1300, 2100), or not supplied (e.g., 0 or NA) collecting year.
...
![Summary of all tests of the time step; note that some database lack event date information](https://github.com/brunobrr/bdc/raw/master/inst/extdata/example_figures/time_.summary_BAR.png){width="20cm"}
...
Let's remove potentially erroneous or suspect records flagged by the data quality tests applied in all steps of the `bdc` package to get a "clean", "fitness-for-use" database. Note that **29%** (2,631 out of 9.000 records) of original records were considered "fitness-for-use" after the data-cleaning process.
