...
Before running the `bdc_clean_names` function, please, install the `gnparser` software by [**following this tutorial**](https://brunobrr.github.io/bdc/articles/help/installing_gnparser.html)**.** More information on gnparser is available on the [software's GitHub](https://github.com/gnames/gnparser).
...
5.  Parse names, i.e., separate author, date, annotations from taxon name using gnpaser.
...
You can use [qs2::qs_save()]{.underline} instead of write_csv to save a large database in a compressed format.

```{r eval=FALSE}
# use qs2::qs_save() to save the database in a compressed format and then qs:qread() to load the database
database %>%
  readr::write_csv(.,
